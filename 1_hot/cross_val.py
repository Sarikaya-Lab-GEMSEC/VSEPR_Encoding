...
def cross_val(args):
    ...
    if args.visualizeNet == True:
        x = Variable(torch.randn(1, 20, 1, 13))

        if args.onGPU == True:
            x = x.cuda()

        if args.model == 'shallow_net':
            model = net.shallow_net().cuda()
        elif args.model == 'LSTM_net':
            model = net.LSTM_net().cuda()
        elif args.model == 'GRU_net':
            model = net.GRU_net().cuda()
        else:
    ...
    if os.path.isfile(logFileLoc):
        ...
        logger.write("%s\t%s\t\t\t\t\t%s\t\t\t%s\n" % ('Length', 'Allele', 'Pearson', 'r2_score'))
        logger.flush()
    else:
            ...
            if (len(sequ) > 0):
                ...
                for train_set, test_set in kf.split(sequ_ori, label_ori):
                    ...
                    if os.path.isfile(alleleLoc):
                        ...
                        log.write("\n")
                        log.write("%s\t\t\t%s\n" % ('Length: ', length))
                        log.write("%s\t\t\t\t%s\t\t\t%s\t\t\t%s\t\t\t%s\t\t\t%s\n" % ('Epoch', 'tr_loss', 'val_loss', 'tr_MSE', 'val_MSE', 'val_Pearson'))
                        log.flush()
                    else:
                    ...
                    for epoch in range(start_epoch, args.max_epochs):
                        ...
                        log.write("%s\t\t\t\t%.4f\t\t\t%.4f\t\t\t\t%.4f\t\t\t%.4f\t\t\t%.4f\n" % (epoch, tr_epoch_loss, val_epoch_loss,tr_mean_squared_error, val_mean_squared_error, val_Pearson[0]))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--savedir', default='./results_shallow_final')
